...
export default class AddNote extends React.Component {
  ...
  handleForm = (e) => {
    ...
    const folder = this.context.folders.find(
      (folder) => folder.title === e.target.folderSelection.value
    );
    const now = new Date();
  ...
  render() {
    ...
    return (
      <form classtitle="newNote" onSubmit={(e) => this.handleForm(e)}>
        ...
        />
        {!this.state.title.touched ? <IsRequired /> : null}
        {this.state.title.touched && <FormError message={titleError} />}

        <label htmlFor="noteContent" classtitle="inputDescription">
          Content
        </label>
        <input
          type="text"
